...
export const URINAL_Y = 372;
export const URINAL_H = 118;
export const MIRROR = { y: 176, h: 56 };
...
export interface Scene {
  ...
  hint: { title: string; text: string; side: 'left' | 'right' } | null;
...
export function slotW(n: number): number {
  return Math.min(62, ((W - LEFT - RIGHT) / n) * 0.82);
}
...
function drawRoom(ctx: CanvasRenderingContext2D, th: Theme, t: number): void {
  ...
  // Farbband auf Augenhöhe der Wand, Sockel unten
  ctx.fillStyle = th.band; ctx.fillRect(0, 136, W, 12);
  ctx.fillStyle = th.skirting; ctx.fillRect(0, FLOOR_Y - 10, W, 10);
  // Deckenlampe
  const flicker = th.dark ? (Math.sin(t * 40) * (Math.sin(t * 0.7) > 0.92 ? 1 : 0)) : 0;
  ctx.fillStyle = th.light; ctx.globalAlpha = 0.85 + 0.15 * flicker;
  rr(ctx, 70, 92, W - 140, 8, 4); ctx.fill();
  ctx.globalAlpha = th.dark ? 0.12 : 0.22;
  ctx.beginPath(); ctx.moveTo(70, 100); ctx.lineTo(W - 70, 100); ctx.lineTo(W - 10, FLOOR_Y); ctx.lineTo(10, FLOOR_Y); ctx.closePath(); ctx.fill();
  ctx.globalAlpha = 1;
  if (th.dark) { ctx.fillStyle = 'rgba(0,0,0,0.25)'; ctx.fillRect(0, 0, W, H); }
  // Boden mit Fluchtpunkt
  ctx.fillStyle = th.floor; ctx.fillRect(0, FLOOR_Y, W, H - FLOOR_Y);
  const vpX = W / 2, vpY = FLOOR_Y - 220;
  ctx.strokeStyle = th.floorLine; ctx.lineWidth = 1;
  for (let i = -7; i <= 7; i++) {
    const xb = W / 2 + i * 62;
    ctx.beginPath(); ctx.moveTo(vpX + (xb - vpX) * ((FLOOR_Y - vpY) / (H - vpY)), FLOOR_Y); ctx.lineTo(xb, H); ctx.stroke();
  }
  for (let k = 0; k < 8; k++) {
    const y = FLOOR_Y + (H - FLOOR_Y) * (1 - Math.pow(1 - k / 8, 1.8));
    ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(W, y); ctx.stroke();
  }
  // Wandschatten auf dem Boden, Abfluss
  const g = ctx.createLinearGradient(0, FLOOR_Y, 0, FLOOR_Y + 40);
  g.addColorStop(0, 'rgba(0,0,0,0.22)'); g.addColorStop(1, 'rgba(0,0,0,0)');
  ctx.fillStyle = g; ctx.fillRect(0, FLOOR_Y, W, 40);
  ctx.fillStyle = 'rgba(0,0,0,0.25)'; ctx.beginPath(); ctx.ellipse(W / 2, FLOOR_Y + 150, 16, 7, 0, 0, Math.PI * 2); ctx.fill();
  ctx.strokeStyle = 'rgba(0,0,0,0.3)'; ctx.beginPath(); ctx.moveTo(W / 2 - 10, FLOOR_Y + 150); ctx.lineTo(W / 2 + 10, FLOOR_Y + 150); ctx.moveTo(W / 2, FLOOR_Y + 146); ctx.lineTo(W / 2, FLOOR_Y + 154); ctx.stroke();
  // Tür in voller Höhe mit Rahmen
  ctx.fillStyle = th.doorDark; rr(ctx, 0, 176, 78, FLOOR_Y - 176, 4); ctx.fill();
  ctx.fillStyle = th.door; rr(ctx, 6, 184, 66, FLOOR_Y - 184, 3); ctx.fill();
  ctx.fillStyle = 'rgba(0,0,0,0.12)'; rr(ctx, 14, 196, 50, 130, 3); ctx.fill(); rr(ctx, 14, 340, 50, 160, 3); ctx.fill();
  ctx.fillStyle = '#d9c9a6'; ctx.beginPath(); ctx.arc(60, 362, 5, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = 'rgba(255,255,255,0.75)'; ctx.font = '800 10px Inter, system-ui, sans-serif';
  ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText('EXIT', 39, 166);
  drawProp(ctx, th, t);
  // Spiegelstreifen auf Augenhöhe
  ctx.fillStyle = th.mirror;
  rr(ctx, 92, MIRROR.y, W - 106, MIRROR.h, 6); ctx.fill();
  ctx.strokeStyle = 'rgba(255,255,255,0.5)'; ctx.lineWidth = 2; ctx.stroke();
  ctx.fillStyle = 'rgba(255,255,255,0.10)';
  ctx.beginPath(); ctx.moveTo(102, MIRROR.y + 4); ctx.lineTo(152, MIRROR.y + 4); ctx.lineTo(122, MIRROR.y + MIRROR.h - 4); ctx.lineTo(98, MIRROR.y + MIRROR.h - 4); ctx.closePath(); ctx.fill();
}
...
export const STALL = { x: W - 74, y: 246, w: 66, h: FLOOR_Y - 246 };
...
function drawUrinal(ctx: CanvasRenderingContext2D, x: number, w: number, slot: Slot, highlight: number, t: number, th: Theme): void {
  const y = URINAL_Y, h = URINAL_H;
  ctx.fillStyle = '#8a9ba1'; ctx.fillRect(x - 6, y - 64, 12, 64);
  ctx.fillStyle = '#b8c6ca'; ctx.fillRect(x - 4, y - 64, 4, 64);
  ctx.fillStyle = '#c9d6da'; rr(ctx, -14 + x, y - 80, 28, 20, 6); ctx.fill();
  ctx.strokeStyle = 'rgba(60,80,90,0.35)'; ctx.lineWidth = 1.5; ctx.stroke();
  ctx.fillStyle = '#e6eef0'; rr(ctx, x - 7, y - 90, 14, 12, 3); ctx.fill();
  ...
  ctx.fillStyle = 'rgba(60,80,90,0.35)'; ctx.beginPath(); ctx.arc(x, y + h * 0.78, 3.5, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = '#8a9ba1'; ctx.fillRect(x - 3, y + h - 2, 6, FLOOR_Y - 10 - (y + h - 2));
  if (slot.kind === 'broken') {
    ctx.fillStyle = '#ffd23f'; rr(ctx, x - w / 2 - 4, y + 30, w + 8, 34, 3); ctx.fill();
    ctx.strokeStyle = '#1a1a1a'; ctx.lineWidth = 2; ctx.stroke();
    ctx.fillStyle = '#1a1a1a'; ctx.font = '800 9px Inter, system-ui, sans-serif';
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText('OUT OF', x, y + 42); ctx.fillText('ORDER', x, y + 54);
  ...
  if (slot.kind === 'wet') {
    ...
    ctx.beginPath(); ctx.ellipse(x + 6, FLOOR_Y + 16, 36, 10, 0, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(x - 14, FLOOR_Y + 28, 20, 7, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = `rgba(255,255,255,${0.5 + 0.3 * Math.sin(t * 3)})`;
    ctx.beginPath(); ctx.ellipse(x + 10, FLOOR_Y + 14, 8, 3, 0, 0, Math.PI * 2); ctx.fill();
    // Warnschild
    ctx.fillStyle = '#ffd23f'; ctx.beginPath(); ctx.moveTo(x + 30, FLOOR_Y + 40); ctx.lineTo(x + 44, FLOOR_Y + 4); ctx.lineTo(x + 58, FLOOR_Y + 40); ctx.closePath(); ctx.fill();
    ctx.strokeStyle = '#1a1a1a'; ctx.lineWidth = 1.5; ctx.stroke();
    ctx.fillStyle = '#1a1a1a'; ctx.font = '800 7px Inter, sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText('WET', x + 44, FLOOR_Y + 30);
  }
  if (highlight > 0) {
...
export function drawPerson(ctx: CanvasRenderingContext2D, x: number, p: Person | null, t: number, opts: { player?: boolean; walk?: number; look?: number; shame?: number; wave?: boolean; view?: 'back' | 'front'; nod?: number; cos?: Cosmetics } = {}): void {
  ctx.save(); ctx.translate(x, FLOOR_Y - 6); ctx.scale(CHAR_SCALE, CHAR_SCALE);
  drawCharacter(ctx, 0, 0, p ?? PLAYER, opts.view ?? 'back', t, { walk: opts.walk, look: opts.look, shame: opts.shame, wave: opts.wave, nod: opts.nod, idle: p ? p.id * 1.7 : 0 }, opts.cos ?? {}, !!opts.player);
  ctx.restore();
}
...
export function render(ctx: CanvasRenderingContext2D, sc: Scene): void {
  ...
  sc.slots.forEach((slot, i) => {
    ...
    if (sc.reactKind === 'bad' && sc.good.includes(i) && sc.reactT > 0.4) {
      ...
      ctx.fillText('HERE', slotX(n, i), URINAL_Y - 112); 
      ctx.beginPath(); ctx.moveTo(slotX(n, i) - 6, URINAL_Y - 104); ctx.lineTo(slotX(n, i), URINAL_Y - 96); ctx.lineTo(slotX(n, i) + 6, URINAL_Y - 104); ctx.closePath(); ctx.fill();
    }
  });
  ...
  if (sc.wanderer && !sc.wanderer.settled) {
    ...
    if (slot.kind === 'taken') {
      ...
      ctx.fillText('WAIT FOR HIM', W / 2, MIRROR.y - 14);
    }
  }
  ...
  if (sc.mirror && !sc.mirror.done) {
    ...
    if (slot.kind === 'taken') {
      ctx.save();
      ctx.beginPath(); ctx.roundRect(92, MIRROR.y, W - 106, MIRROR.h, 6); ctx.clip();
      ctx.globalAlpha = 0.9 * k;
      ctx.translate(x, MIRROR.y + MIRROR.h / 2 + 142 * 0.6 + 100 * (1 - k));
      ctx.scale(0.6, 0.6);
      ...
      ctx.beginPath(); ctx.moveTo(x, MIRROR.y + MIRROR.h / 2); ctx.lineTo(px, MIRROR.y + MIRROR.h / 2); ctx.stroke(); ctx.setLineDash([]);
      ctx.fillStyle = '#1a2a30'; ctx.font = '800 13px Inter, system-ui, sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
      ctx.fillText(sc.mirror.wantsNod ? 'YOUR MATE IS LOOKING AT YOU' : "HE'S LOOKING AT YOU IN THE MIRROR", W / 2, MIRROR.y - 14);
      ctx.fillStyle = 'rgba(26,42,48,0.3)'; ctx.fillRect(W / 2 - 60, MIRROR.y + MIRROR.h + 8, 120, 4);
      ctx.fillStyle = '#ffd23f'; ctx.fillRect(W / 2 - 60, MIRROR.y + MIRROR.h + 8, 120 * Math.max(0, 1 - sc.mirror.t / 3), 4);
    }
  }
  ...
  if (sc.moveMode) {
    ctx.fillStyle = 'rgba(26,42,48,0.88)'; rr(ctx, W / 2 - 130, 112, 260, 46, 12); ctx.fill();
    ctx.fillStyle = '#fff'; ctx.font = '800 14px Inter, system-ui, sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText('MOVE?', W / 2, 127);
    ctx.font = '600 12px Inter, system-ui, sans-serif'; ctx.fillText('Tap another urinal, or tap yourself to STAY', W / 2, 145);
    const pulse = 0.5 + 0.5 * Math.sin(sc.t * 6);
    ctx.strokeStyle = `rgba(255,210,63,${0.5 + 0.5 * pulse})`; ctx.lineWidth = 3;
    ctx.beginPath(); ctx.ellipse(sc.playerX, FLOOR_Y - 2, 34, 10, 0, 0, Math.PI * 2); ctx.stroke();
    ctx.fillStyle = '#ffd23f'; ctx.font = '800 12px Inter, system-ui, sans-serif'; ctx.fillText('STAY', sc.playerX, FLOOR_Y + 22);
  }
  if (sc.hint) {
    ctx.fillStyle = 'rgba(26,42,48,0.9)'; rr(ctx, 16, FLOOR_Y + 40, W - 32, 74, 14); ctx.fill();
    ctx.fillStyle = '#ffd23f'; ctx.font = '800 15px Inter, system-ui, sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText(sc.hint.title, W / 2, FLOOR_Y + 62);
    ctx.fillStyle = '#fff'; ctx.font = '600 13px Inter, system-ui, sans-serif'; ctx.fillText(sc.hint.text, W / 2, FLOOR_Y + 86);
    // Pfeil zum Knopf unten links oder rechts
    const ax = sc.hint.side === 'left' ? 70 : W - 70;
    const by = H - 70 + Math.sin(sc.t * 5) * 5;
    ctx.fillStyle = '#ffd23f'; ctx.beginPath(); ctx.moveTo(ax - 12, by - 26); ctx.lineTo(ax + 12, by - 26); ctx.lineTo(ax, by - 8); ctx.closePath(); ctx.fill();
  }
